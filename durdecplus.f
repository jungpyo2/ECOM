...
CDIR$ IVDEP
  ...
  130 CONTINUE
      RETURN
      END
ccc      SUBROUTINE DURDEC(F,C,COSCOE,SINCOE,W,WSAVE)
ccc      COMPLEX *16 F(1),C,COSCOE(1),SINCOE(1),W(1),WSAVE(1),IMA,
ccc  1              IM05,ALK,BETK,AK,BK
ccc     REAL *8 DHALF,D1
c       DATA IMA/(0.0D0,1.0D0)/,DHALF/0.5D0/
C
...
C
C
ccc     ENTRY DURINI(WSAVE,N77)
ccc     N=N77
ccc     CALL DCFFTI(N,WSAVE)
ccc     NHALF= N   /2
ccc     IM05=IMA*0.5*2.0/N
ccc     D1=1
ccc     D1=D1/N
ccc     RETURN
C
...
C
C
ccc     ENTRY DURSYN(C,COSCOE,SINCOE,F,WSAVE)
C
C        MERGE SINES AND COSINES INTO EXPONENTIIALS
C
ccc     F(1)=C
ccc     DO 2200 I=1,NHALF
ccc     I1=I+1
ccc     IN1=N-I+1
ccc     AK=COSCOE(I)*DHALF
ccc     BK=SINCOE(I)*DHALF
ccc     F(I1)=AK-IMA*BK
ccc     F(IN1)=AK+IMA*BK
c2200 CONTINUE
C
...
c---------------
C
C     Fourier differentiation for periodic function given ITS
...
c---------------
c
      SUBROUTINE FDIFF2C(FVALUE,FDERIV,NPTS,WORK)
c
c---------------
C
C     Fourier differentiation for periodic function given ITS
...
c
         DO k = -nsc,-1
 	    FDERIV(npts+k+1) = (k*EYE)**2*FVALUE(npts+k+1)
         end do
         FDERIV(1) = 0.d0
         DO k = 1,NSCR
	    FDERIV(k+1) = (k*EYE)**2*FVALUE(k+1)
         end do
c
         CALL DCFFTB(NPTS,FDERIV,WORK)
c
...
c---------------
c
      SUBROUTINE FCOFFTH(FVALUE,NPTS,THIN,FOUT)
c
C
      IMPLICIT REAL *8 (A-H,O-Z)
      INTEGER *4 NPTS,LW,IPT,NSC
      COMPLEX *16 FVALUE(NPTS)
      COMPLEX *16 FOUT,EYE
C
      EYE = DCMPLX(0.0d0,1.0d0)
C
...
c         write(*,*) 'thinf',thin
         FOUT = (0.0d0, 0.0d0)
         DO k = -nsc,-1
            FOUT=FOUT+FVALUE(npts+k+1)*cdexp(k*EYE*THIN)
         end do
         FOUT=FOUT+FVALUE(1)
         DO k = 1,NSCR
            FOUT=FOUT+FVALUE(k+1)*cdexp(k*EYE*THIN)
c            write(*,*) 'FOUT',FOUT,FVALUE(k+1)*cdexp(k*EYE*THIN)
         end do
c
c
      return
...
c---------------
c
      SUBROUTINE FDIFFCOFFTH(FVALUE,FDERIV,NPTS,THIN,FOUT)
c
c---------------
C
C     Fourier differentiation for periodic function given ITS
...
c
         ...
         DO k = -nsc,-1
 	    FDERIV(npts+k+1) = k*EYE*FVALUE(npts+k+1)
            FOUT=FOUT+FDERIV(npts+k+1)*cdexp(k*EYE*THIN)
         end do
         FDERIV(1) = 0.d0
         DO k = 1,NSCR
	    FDERIV(k+1) = k*EYE*FVALUE(k+1)
            FOUT=FOUT+FDERIV(k+1)*cdexp(k*EYE*THIN)
         end do
c
c
      return
...
c---------------
c
      SUBROUTINE FDIFF2COFFTH(FVALUE,FDERIV,NPTS,THIN,FOUT)
c
c---------------
C
C     Fourier differentiation for periodic function given ITS
...
C
         EYE = DCMPLX(0.0d0,1.0d0)
c
         call DCFFTF (npts,fvalue,work)
         do i = 1,npts
            fvalue(i) = fvalue(i)/npts
         end do
C
c---- copy FVALUE into workspace
c
